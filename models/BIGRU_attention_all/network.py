...
def getweight():
    df_topic = pd.read_csv('../../raw_data/topic_2_id_new.csv')
...
class Settings(object):
    def __init__(self):
        self.model_name = 'BIGRU_attention_all_s'
        self.title_len = 30
        self.content_len = 200
        self.hidden_size = 200
        self.n_layer = 1
        self.fc_hidden_size = 1024
        self.n_class = 25551
        self.summary_path = '../../summary/' + self.model_name + '/'
        self.ckpt_path = '../../ckpt_all/' + self.model_name + '/'
...
class BiGRU(object):
    ...
    def __init__(self, W_embedding, settings):
        ...
        with tf.name_scope('Inputs'):
            ...
            self._X1_length = tf.placeholder(tf.int64, [None], name='X1_length')
            self._X2_length = tf.placeholder(tf.int64, [None], name='X2_length')
        ...
        with tf.variable_scope('bigru_text'):
            output_title = self.bigru_inference(self._X1_inputs, self._X1_length)

        with tf.variable_scope('bigru_content'):
            output_content = self.bigru_inference(self._X2_inputs, self._X2_length)
        ...
        with tf.name_scope('loss'):
            #pos_weight = getweight()
            self._loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=self._y_pred, labels=self._y_inputs)
                #tf.nn.weighted_cross_entropy_with_logits(logits=self._y_pred, targets=self._y_inputs, pos_weight=pos_weight)
                )
    ...
    @property
    def X1_length(self):
        return self._X1_length

    @property
    def X2_length(self):
        return self._X2_length
    ...
    def bi_gru(self, inputs,X_length):
    ...
    def bigru_inference(self, X_inputs, X_length):
        inputs = tf.nn.embedding_lookup(self.embedding, X_inputs)
        output_bigru = self.bi_gru(inputs,X_length)
