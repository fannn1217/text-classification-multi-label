...
class RCNN(object):
    def __init__(self, W_embedding, settings):
        ...
        with tf.variable_scope('rcnn_text'):
            output_title = self.rcnn_inference(self._X1_inputs, self.title_len, self._X1_length)

        with tf.variable_scope('rcnn_content'):
            output_content = self.rcnn_inference(self._X2_inputs, self.content_len, self._X2_length)
