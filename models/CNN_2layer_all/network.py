...
class Settings(object):
    def __init__(self):
        self.model_name = 'CNN-2layer-all'
        self.title_len = 20
...
class TextCNN(object):
    ...
    def __init__(self, W_embedding, settings):
        ...
        with tf.variable_scope('embedding'):
            #随机初始化
            #self.embedding = tf.get_variable(name='embedding', shape=[575746,200], trainable=True)#字典size
            self.embedding = tf.get_variable(name='embedding', shape=W_embedding.shape,initializer=tf.constant_initializer(W_embedding), trainable=True)
    ...
    def cnn_inference(self, X_inputs, n_step):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            ...
            with tf.variable_scope("conv2%s" % filter_size):
                filter_shape = [filter_size, 1, self.n_filter, self.n_filter]
                W_filter = self.weight_variable(shape=filter_shape, name='W_filter')
                beta = self.bias_variable(shape=[self.n_filter], name='beta_filter')
               # tf.summary.histogram('beta', beta)
