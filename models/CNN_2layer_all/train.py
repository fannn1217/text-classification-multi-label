...
sys.path.append('../..')
from raw_data.data_helpers import to_categorical
from raw_data.evaluator import score_eval
...
flags.DEFINE_integer('max_max_epoch', 2, 'all training epoches, default: 6')
flags.DEFINE_float('lr', 5e-5, 'initial learning rate, default: 1e-3')
...
flags.DEFINE_float('last_f1', 0.2, 'if valid_f1 > last_f1, save new model. default: 0.40')
...
data_train_path = '../../data_new/wd-data_all_120_s/data_train/'
data_valid_path = '../../data_new/wd-data_all_120_s/data_dev/'
...
def main(_):
    ...
    config = tf.ConfigProto(allow_soft_placement=True)#gpu配置
    ...
    with tf.Session(config=config) as sess:
        ...
        with tf.variable_scope('training_ops') as vs:
            learning_rate = tf.train.exponential_decay(FLAGS.lr, model.global_step, FLAGS.decay_step,
                                                   FLAGS.decay_rate, staircase=True)#学习率指数衰减
            # two optimizer: op1, update embedding; op2, do not update embedding.
            with tf.variable_scope('Optimizer1'):
                tvars1 = tf.trainable_variables()#需要训练的变量
                grads1 = tf.gradients(model.loss, tvars1)#计算梯度
        ...
        if f1>last_f1:
            ...
            print('saved final model to %s ' % saving_path)
